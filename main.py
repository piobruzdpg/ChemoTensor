...
class FileManagerWindow(ctk.CTkToplevel):
    def __init__(self, parent, file_list, callback_apply):
        ...
        self.parent = parent
        self.file_list = file_list # Lista słowników: {'path': str, 'name': str, 'data': array}
        self.callback_apply = callback_apply # Funkcja wywoływana po zatwierdzeniu
        self.selected_index = None
        
        self.file_manager_window = None
        self.loaded_files_buffer = [] # Tu będziemy trzymać listę plików

        # UI Layout
        self.grid_columnconfigure(0, weight=1)
        self.grid_columnconfigure(1, weight=0) # Panel boczny stały
        ...
        ctk.CTkButton(self.bottom_frame, text="Apply to Grid", command=self._on_apply, fg_color="#2ECC71", hover_color="#27AE60").pack(fill="x", pady=5)
    ...
    def _setup_controls(self):
        ...
        ctk.CTkButton(self.controls_frame, text="Add Placeholder", command=self._add_placeholder, fg_color="#E67E22", hover_color="#D35400", **btn_opts).pack(pady=5)
        ctk.CTkLabel(self.controls_frame, text="---").pack(pady=2)
        ctk.CTkButton(self.controls_frame, text="Move Up ▲", command=lambda: self._move_item(-1), **btn_opts).pack(pady=5)
        ctk.CTkButton(self.controls_frame, text="Move Down ▼", command=lambda: self._move_item(1), **btn_opts).pack(pady=5)
        ctk.CTkLabel(self.controls_frame, text="---").pack(pady=2)
        ctk.CTkButton(self.controls_frame, text="Sort A-Z", command=self._sort_az, **btn_opts).pack(pady=5)
        ctk.CTkButton(self.controls_frame, text="Remove Selected", command=self._remove_selected, fg_color="#C0392B", hover_color="#E74C3C", **btn_opts).pack(pady=5)
    ...
    def _refresh_list(self):
        ...
        for i, item in enumerate(self.file_list):
            ...
            if i == self.selected_index:
                bg_color = ("#3B8ED0", "#1F6AA5") # Kolor Theme Blue
            
            # Kontener wiersza
            row = ctk.CTkFrame(self.scroll_frame, fg_color=bg_color)
            row.pack(fill="x", pady=1)
            
            # Kliknięcie w ramkę zaznacza
            row.bind("<Button-1>", lambda e, idx=i: self._select_item(idx))
            
            # Etykieta numeru i nazwy
            name = item['name']
            if item.get('is_placeholder'):
                name = "[ EMPTY PLACEHOLDER ]"
                text_color = "orange"
            else:
                text_color = "text_color" # domyślny
                
            lbl = ctk.CTkLabel(row, text=f"{i+1}. {name}", anchor="w", text_color=text_color if text_color != "text_color" else None)
            lbl.pack(fill="x", padx=5, pady=2)
            
            # Przekazywanie kliknięcia z labela na ramkę
    ...
    def _add_files(self):
        ...
        # Pobieramy referencję do aplikacji rodzica, żeby sprawdzić wymiary
        # Zakładamy, że self.parent to ChemTensorApp
        ...
        for p in paths:
            try:
                ...
                # Sprawdzenie wymiarów (jeśli to nie pierwsze pliki w ogóle)
                if ref_wav is not None and not np.array_equal(ref_wav, wav):
                    messagebox.showwarning("Dimension Mismatch", f"File {os.path.basename(p)} has different wavenumbers. Skipped.")
                    continue
                
                # Jeśli to absolutnie pierwszy plik w projekcie, ustawiamy go jako wzorzec w rodzicu
                if ref_wav is None:
    ...
    def _sort_az(self):
        # Sortujemy, ale placeholder'y trafiają na koniec lub zostają (zależy od logiki, tu prosto po nazwie)
...
class ToolTip(object):
    """
    Tworzy dymek z podpowiedzią (tooltip) dla dowolnego widgetu.
    """
    ...
    def enter(self, event=None):
        # Czekaj 1000ms (1s) przed pokazaniem
        self.schedule()
    ...
    def schedule(self):
        self.unschedule()
        self.id = self.widget.after(1000, self.show) # 1000 ms = 1 sekunda
    ...
    def show(self):
        ...
        # Pobierz pozycję myszy/widgetu
        x, y, cx, cy = self.widget.bbox("insert")
        x += self.widget.winfo_rootx() + 25
        y += self.widget.winfo_rooty() + 25
        
        # Stwórz okno bez ramek (Toplevel)
        self.tooltip_window = tw = tkinter.Toplevel(self.widget)
        tw.wm_overrideredirect(True) # Brak dekoracji okna (X, minimalizuj itp.)
...
class ChemTensorApp(ctk.CTk):
    def __init__(self):
        ...
        self.offset_target_val_var = ctk.StringVar(value="0.0") # Domyślnie 0.0
        self.offset_point_var = ctk.StringVar(value="")         # Domyślnie puste
        self.show_preprocessed_var = ctk.BooleanVar(value=False)
        self.pipeline_steps = []  # List of dicts: {'type': 'SNV', 'params': {...}}
        ...
        self.tensor_data = None      # Working data (after crop)
        self.wavenumbers = None      # X-axis (after crop)
        ...
        self.last_analysis_mode = None  # Remembers last used analysis
        
        # Plot Tools
        self.zoom_rects = {}    # Stores rectangles
        self.zoom_start = {}    # Stores start point (x, y)
        self.initial_lims = {}
        self.pls_target_map = {}
    ...
    def _on_tab_change(self):
        """Automatically changes view depending on the active tab."""
        tab_name = self.tab_view.get()

        if tab_name == "Data":
            # Show raw data (disable preprocessing view)
            self.show_preprocessed_var.set(False)
            self.current_plot_mode = 'SPECTRA'
            
        elif tab_name == "Preprocessing":
            # Show processed data (enable preview)
            self.show_preprocessed_var.set(True)
            self.current_plot_mode = 'SPECTRA'
            
        elif tab_name == "Analysis":
            # Restore last analysis (if any), otherwise spectra
    ...
    def _setup_data_tab(self, parent):
        ...
        ctk.CTkButton(row2, text="Manage Files", command=self._open_file_manager).pack(side="left", fill="x", expand=True, padx=2)
        ctk.CTkButton(row2, text="Select Active", command=self._select_all_active).pack(side="left", fill="x", expand=True, padx=2)
        ctk.CTkButton(row2, text="Deselect", command=self._deselect_all).pack(side="left", fill="x", expand=True, padx=2)

        # Range
        row3 = ctk.CTkFrame(frame)
        row3.pack(fill="x", pady=5)

        # 1. Elementy stałe (Label i Entry) - pakujemy do lewej
        ...
        # 2. Kontener na przyciski (wypełnia resztę miejsca)
        # fg_color="transparent" sprawia, że ramka jest niewidoczna
        buttons_frame = ctk.CTkFrame(row3, fg_color="transparent")
        buttons_frame.pack(side="left", fill="x", expand=True, padx=5)

        # Konfiguracja siatki wewnątrz kontenera przycisków:
        # Dwie kolumny o wadze 1 (dzielą miejsce po równo)
        buttons_frame.grid_columnconfigure(0, weight=1)
        buttons_frame.grid_columnconfigure(1, weight=1)

        # 3. Przyciski w siatce (sticky="ew" rozciąga je na boki)
        ctk.CTkButton(buttons_frame, text="Apply", command=self._apply_wavenumber_range).grid(row=0, column=0, sticky="ew", padx=2)
        ctk.CTkButton(buttons_frame, text="Reset", command=self._reset_wavenumber_range).grid(row=0, column=1, sticky="ew", padx=2)
    ...
    def _open_file_manager(self):
        if self.file_manager_window is None or not self.file_manager_window.winfo_exists():
            # Przekazujemy obecną listę (jeśli istnieje) lub pustą
    ...
    def _apply_file_list_to_grid(self, file_list):
        """Mapuje listę plików na siatkę NxM."""
        self.loaded_files_buffer = file_list # Zapisz stan listy
        ...
        # Inicjalizacja tensora jeśli nie istnieje (na podstawie pierwszego nie-placeholdera)
        if self.original_tensor_data is None:
            # Znajdź pierwszy poprawny plik, żeby pobrać długość widma
            ...
            if first_valid:
                ...
                return # Same placeholdery? Nic nie robimy
        ...
        for r in range(n):
            for c in range(m):
                ...
                # Jeśli skończyły się pliki na liście, przerywamy lub czyścimy resztę
                if idx >= len(file_list):
                    self._mark_cell_as_empty(coords) # lub zostawiamy jak było
                else:
                    item = file_list[idx]

                    if item.get('is_placeholder'):
                        # To jest placeholder -> oznacz jako pusty
                        self._mark_cell_as_missing(coords) # Lub Empty, zależnie od logiki
                        # Ale chcemy, żeby w "statusie" było widać, że to celowy placeholder
                        self._update_cell_status(coords, 'MISSING', f"{coords}\n[SKIP]")
                    else:
                        # To jest plik
                        absorb = item['data']
                        name = item['name']
                        # Skracanie nazwy do wyświetlenia
                        suffix_len = 6
                        ...
                        # Dodaj tooltip (jeśli widget istnieje)
    ...
    def _setup_visual_tab(self, parent):
        frame = ctk.CTkScrollableFrame(parent, fg_color="transparent")
        frame.pack(fill="both", expand=True)
        ctk.CTkSwitch(frame, text="Show processed data", variable=self.show_preprocessed_var, command=self.update_plot).pack(pady=10)
    ...
    def _setup_preprocess_tab(self, parent):
        ...
        # TOP: Button Panel (Scrollable to fit)
        ...
        # Tooltip wyjaśniający p
        ToolTip(lbl_als_p, text="Asymmetry (p).\nWeight for positive residuals (0 < p < 1).\nTypically small (0.001 - 0.05).\nControls how strictly the baseline stays below the peaks.")
        ...
        # Entry Value (target y, default 0.0)
        ...
        ctk.CTkButton(grid_norm, text="Area", command=self._apply_l1_norm, width=60).grid(row=0, column=2, padx=2, pady=2, sticky="ew")
        ...
        ToolTip(lbl_sm_w, text="Window length (must be odd integer)")
        ...
        ToolTip(lbl_sm_p, text="Polynomial order")
        
        # Entry p
        ctk.CTkEntry(row_smooth, textvariable=self.sg_smooth_poly_var, width=40).pack(side="left", padx=2)
        
        # Button (fill="x" takes remaining space)
        ctk.CTkButton(row_smooth, text="Add", command=self._apply_sg_smoothing).pack(side="left", fill="x", expand=True, padx=5)

    # 4. 2nd Deriv. (Savitzky-Golay)
        ...
        ToolTip(lbl_der_w, text="Window length (must be odd integer)")
        ...
        ToolTip(lbl_der_p, text="Polynomial order")
        
        # Entry p
        ctk.CTkEntry(row_sg, textvariable=self.sg_poly_var, width=40).pack(side="left", padx=2)
        
        # Button
        ctk.CTkButton(row_sg, text="Add", command=self._apply_sg_filter).pack(side="left", fill="x", expand=True, padx=5)
    ...
    def _setup_analysis_tab(self, parent):
            ...
            # PFA (Rank Analysis)
            ctk.CTkButton(fa_frame, text="Rank Analysis (RE/IND)", command=self._run_fa_rank_analysis).pack(fill="x", pady=2)
            ...
            ctk.CTkButton(row_fa_rec, text="FA Reconstruction", command=self._run_fa_reconstruction).pack(side="left", fill="x", expand=True)
            ...
            ctk.CTkButton(row_pca_rec, text="PCA Reconstruction", command=self._run_pca_reconstruction).pack(side="left", fill="x", expand=True)
            ...
            ctk.CTkButton(row_mcr2, text="Known Spectra...", command=self._load_mcr_st_init).pack(side="left", fill="x", expand=True)
            ctk.CTkEntry(row_mcr2, textvariable=self.mcr_st_fix_var, placeholder_text="Fix (e.g. 0,2)").pack(side="left", fill="x", expand=True)
            ...
            ctk.CTkButton(row_tens_rec, text="from PARAFAC", command=lambda: self._run_tensor_recon('parafac')).pack(side="left", padx=2)
            ctk.CTkButton(row_tens_rec, text="from MCR", command=lambda: self._run_tensor_recon('mcr')).pack(side="left", padx=2)

            # 5. 2D-COS
            cos_frame = ctk.CTkFrame(frame)
            cos_frame.pack(fill="x", pady=5)
            ctk.CTkLabel(cos_frame, text="2D-COS", font=("Arial", 12, "bold")).pack()
            ctk.CTkOptionMenu(cos_frame, variable=self.cos_axis_var, values=["Analyze Rows (N)", "Analyze Columns (M)"]).pack(fill="x", pady=2)
            ctk.CTkButton(cos_frame, text="Run 3D-COS", command=self._run_3dcos).pack(fill="x")
            
            # Hidden slider (shown in update_plot)
...
# --- GRID OPERATIONS (Fill Methods) ---
    ...
    def _fill_cell_with_zeros(self, coords):
        ...
        self._reset_wavenumber_range() # Refresh tensor_data
        self.update_plot()
    ...
    def _fill_cell_with_row_mean(self, coords):
        ...
        try:
            ...
            if np.all(np.isnan(mean_spectrum)): 
                raise ValueError("No data in this row.")
    ...
    def _fill_cell_with_col_mean(self, coords):
        ...
        try:
            ...
            if np.all(np.isnan(mean_spectrum)): 
                raise ValueError("No data in this column.")
    ...
    # --- NEIGHBOR MEAN METHODS ---

    def _fill_cell_with_row_neighbors(self, coords):
        """Fills cell with mean of immediate left and right neighbors."""
        if self.original_tensor_data is None: return
        r, c = coords
    ...
    def _fill_cell_with_col_neighbors(self, coords):
        """Fills cell with mean of immediate top and bottom neighbors."""
        if self.original_tensor_data is None: return
        r, c = coords
    ...
    def _calculate_neighbors_mean(self, coords, neighbors, label):
        """Helper to calculate mean from a list of spectra."""
        try:
            # Filter only valid spectra (not all NaN)
            valid_neighbors = [n for n in neighbors if not np.all(np.isnan(n))]
            
            if not valid_neighbors:
                raise ValueError("No valid neighbors (all empty or NaN).")
        ...
        for coords in cells:
            ...
            try:
                ...
                if np.array_equal(self.original_wavenumbers, wav):
                    ...
                    # --- ZMIANA: Wyciąganie nazwy pliku ---
                    filename = os.path.basename(path)       # Np. "probka_123.csv"
                    name_only = os.path.splitext(filename)[0] # Np. "probka_123"
                    
                    # Logika: ".." + ostatnie 6 znaków
                    suffix_len = 6
                    if len(name_only) > suffix_len:
                        # [-suffix_len:] bierze znaki od końca
                        display_name = ".." + name_only[-suffix_len:] 
                    else:
                        display_name = name_only
                    
                    self._update_cell_status(coords, 'LOADED', f"{coords}\n{display_name}")
                    # ---------------------------------------
                    # --- NOWOŚĆ: Podpięcie Tooltipa ---
                    # Pobieramy obiekt przycisku na podstawie współrzędnych
                    btn_widget = self.field_matrix_widgets[coords]
                    # Tworzymy instancję Tooltipa dla tego przycisku z pełną nazwą pliku
    ...
    def _on_cell_right_click(self, event, coords):
        ...
        # Options available always (except correctly loaded)
        if status != 'LOADED':
        ...
        # Fill options (available if tensor has data)
        if self.original_tensor_data is not None:
            menu.add_separator()
            # Submenu "Fill..."
            ...
            fill_menu.add_separator()
            fill_menu.add_command(label="Neighbor Mean (Left/Right)", command=lambda: self._fill_cell_with_row_neighbors(coords))
            fill_menu.add_command(label="Neighbor Mean (Top/Bottom)", command=lambda: self._fill_cell_with_col_neighbors(coords))
    ...
    def _get_active_data(self):
        # Logic: decided by "Show processed data" switch
    ...
    def _apply_sg_smoothing(self):
        ...
        except ValueError: messagebox.showerror("Error", "Integers required")
    ...
    def _apply_l1_norm(self): self._add_pipeline_step('L1', label="L1 Norm")
    ...
    def _apply_als(self):
        ...
        except ValueError: messagebox.showerror("Error", "Numbers required")
    ...
    def _apply_offset(self):
        try:
            ...
            # Sprawdź, czy liczba falowa jest w zakresie (zgrubnie)
    ...
    def _add_pipeline_step(self, method_type, params=None, label=None):
        """Adds step to list and refreshes result."""
        step = {
            'type': method_type,
    ...
    def _run_pipeline(self):
        """Takes raw data and passes through all steps."""
        ...
        try:
            ...
            # Automatically enable preview if steps added
    ...
    def _refresh_pipeline_ui(self):
        ...
        for i, step in enumerate(self.pipeline_steps):
            ...
            if i < len(self.pipeline_steps) - 1: # Down button
    ...
    def _run_pca(self):
        ...
        try:
            ...
            self.update_plot()
        except Exception as e: messagebox.showerror("PCA Error", f"{e}")
    ...
    def _run_mcr_als(self):
        ...
        try:
            ...
            self.update_plot()
        except Exception as e: messagebox.showerror("MCR Error", f"{e}")
    ...
    def _run_parafac(self):
        ...
        try:
            ...
            self._update_pls_target_options()
            self.update_plot()
        except Exception as e: messagebox.showerror("PARAFAC Error", f"{e}")
    ...
    def _run_umap(self):
        ...
        try:
            ...
            self.update_plot()
        except Exception as e: messagebox.showerror("UMAP Error", f"{e}")
    ...
    def _run_tsne(self):
        ...
        try:
            ...
            self.last_analysis_mode = 'MANIFOLD'
            self.update_plot()
        except Exception as e: messagebox.showerror("t-SNE Error", f"{e}")
    ...
    # --- PLS, 2D-COS, MCR-ADVANCED ---

    def _update_pls_target_options(self):
        """Updates PLS target list (Y) based on MCR/PARAFAC results."""
        ...
        if self.parafac_results:
            ...
            for i in range(k):
                ...
                # Repeat values for flattened tensor match
    ...
    def _run_pls(self):
        ...
        try:
            ...
            self.update_plot()
        except Exception as e: messagebox.showerror("PLS Error", f"{e}")
    ...
    def _run_3dcos(self):
        ...
        except Exception as e: messagebox.showerror("2D-COS Error", f"{e}")
    ...
    def _run_tucker(self):
        ...
        try:
            ...
            self.update_plot()
        except Exception as e: messagebox.showerror("Tucker Error", f"{e}")

    # 3. Factor Analysis (Rank Analysis)
    def _run_fa_rank_analysis(self):
        ...
        try:
            ...
            self.update_plot()
        except Exception as e: messagebox.showerror("FA Error", f"{e}")

    # 4. FA Reconstruction
    def _run_fa_reconstruction(self):
        # --- POPRAWKA: Dodano sprawdzenie z komunikatem ---
    ...
    def _run_spexfa(self):
        ...
        try:
            ...
            self.update_plot()
        except Exception as e: messagebox.showerror("SPEXFA Error", f"{e}")

    # 6. Tensor Reconstruction (PARAFAC/MCR)
    def _run_tensor_recon(self, method):
        # method: 'parafac' or 'mcr'
        data = self._get_active_data()
        if data is None: return
        try:
    ...
    def update_plot(self):
        ...
        if mode in ['SPECTRA', 'HEATMAP', 'PLS_RESULTS', 'MANIFOLD']:
            ...
            if mode == 'PCA' and self.pca_results: vis.plot_pca(self.axes, self.pca_results, self.wavenumbers)
                 ...
                 ax4.bar(range(k), self.parafac_results['weights']); ax4.set_title("Weights")
...
# --- ZOOM HANDLER ---
            ...
            if active_ax and active_ax in self.zoom_rects:
                ...
                if x1 is not None and y1 is not None:
                    if abs(x1 - x0) > 1e-5 or abs(y1 - y0) > 1e-5:
                        ...
                        # Handle inverted axis (crucial for IR!)
        ...
        if f:
            try:
                ...
                data_io.export_results_to_xlsx(
                    ...
                    results,
                    getattr(self, 'pipeline_steps', None)
                )
    ...
    def _on_global_click(self, event):
        """
        Removes focus from buttons when clicking on background.
        Prevents accidental re-triggering with Space/Enter.
        Ignores Entry widgets.
        """
