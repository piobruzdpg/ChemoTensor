...
def plot_pca(axes, results, wavenumbers):
    ...
    if wavenumbers is not None:
        for i in range(n_components):
            alpha_val = max(1.0 - i * 0.2, 0.2)
            ax2.plot(wavenumbers, loadings[:, i], label=f"PC{i + 1}", alpha=alpha_val)
...
def plot_mcr(axes, results, wavenumbers, n_rows, m_cols):
    ...
    if wavenumbers is not None:
        for i in range(k):
            alpha_val = max(1.0 - i * 0.2, 0.2)
            ax1.plot(wavenumbers, ST[:, i], label=f"Comp. {i + 1}", alpha=alpha_val)
...
def plot_tucker(axes, results, wavenumbers, n_rows, m_cols):
    ...
    if wavenumbers is not None:
        for i in range(r_w):
            alpha_val = max(1.0 - i * 0.2, 0.2)
            ax1.plot(wavenumbers, factors[0][:, i], label=f"Comp. {i + 1}", alpha=alpha_val)
...
def plot_fa_rank(axes, results):
    ...
    apply_theme(axes[0, 0]);
    apply_theme(axes[0, 1]);
    apply_theme(axes[1, 0]);
    apply_theme(axes[1, 1])
    ...
    loadings = results['vh'][:max_k]
    for i in range(max_k):
        alpha_val = max(1.0 - i * 0.2, 0.2)
        axes[1, 0].plot(loadings[i, :], alpha=alpha_val)
...
def plot_reconstruction(axes, results, wavenumbers, selected_coords=None):
    ...
    # Bezpieczne pobranie liczby użytych komponentów 'k'
    k_val = results.get('k', results.get('recon_k', None))

    if 'components' in results:  # Tensor
        comps = results['components']
        # Przytnij do użytych komponentów
        if k_val is not None and comps.shape[1] > k_val:
            comps = comps[:, :k_val]

        if wavenumbers is not None:
            for i in range(comps.shape[1]):
                alpha_val = max(1.0 - i * 0.2, 0.2)
                ax4.plot(wavenumbers, comps[:, i], alpha=alpha_val, label=f"Comp {i + 1}")
        else:
            for i in range(comps.shape[1]):
                alpha_val = max(1.0 - i * 0.2, 0.2)
                ax4.plot(comps[:, i], alpha=alpha_val)

    elif 'loadings' in results:  # PCA / FA
        loadings = results['loadings']
        # Przytnij do użytych komponentów
        if k_val is not None and loadings.shape[1] > k_val:
            loadings = loadings[:, :k_val]

        if wavenumbers is not None:
            for i in range(loadings.shape[1]):
                alpha_val = max(1.0 - i * 0.2, 0.2)
                ax4.plot(wavenumbers, loadings[:, i], alpha=alpha_val, label=f"Comp {i + 1}")
        else:
            for i in range(loadings.shape[1]):
                alpha_val = max(1.0 - i * 0.2, 0.2)
                ax4.plot(loadings[:, i], alpha=alpha_val)

    ax4.set_title("Used Components")
    if wavenumbers is not None: invert_xaxis_if_wavenumbers(ax4, wavenumbers)

    # Zabezpieczenie przed błędem z pustą legendą
...
def plot_spexfa(axes, results, wavenumbers):
    ...
    if wavenumbers is not None:
        for i in range(k):
            alpha_val = max(1.0 - i * 0.2, 0.2)
            ax1.plot(wavenumbers, ST[:, i], label=f"Factor {i + 1}", alpha=alpha_val)
